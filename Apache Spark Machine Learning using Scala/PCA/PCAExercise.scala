...
val data = spark.read.option("Header","true").option("inferSchema","true").format("csv").load("Cancer_Data.csv")
...
val scalar = (new StandardScaler().setInputCol("features").setOutputCol("scaledFeatures").setWithStd(true).setWithMean(false))
